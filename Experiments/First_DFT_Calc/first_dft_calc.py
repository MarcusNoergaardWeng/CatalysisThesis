...
adsorbates = ['OH'] #['OH','O']
sites = ['ontop']    #['ontop','fcc']
init_bonds = ['2.0']       #[2.0,1.3]
ads_per_slab = 1
multiple_adsId = None
...
for i in np.arange(start_id,end_id+1):
	...
	try:
		if sys.argv[1] == 'submit':
			...
			while job_id == None:
				try:
					job_id = int(subprocess.run(['sacct', '-n', '-X', '--state=R,PD', '--format=jobid', 
f'--name={filename}_slab'],
			...
			for j, ads in enumerate(adsorbates):
				if multiple_adsId == None:
					...
					for id_set in multiple_adsId:
						...
						SLURM_script(filename + f'_{sites[j]}_{ads}_{ads_id_str}', **SLURM_kwargs, 
dependency=job_id)
