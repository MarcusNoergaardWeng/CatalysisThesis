...
def add_COOH_CO_adsorbed(slab, site_idx=0):
...
with connect('COOH_COH_adsorbed.db') as db_ads,\
	...
	for slab_idx in range(n_slabs):
		...
		for site_idx in range(n_atoms_surface):
			...
			atoms = add_COOH_CO_adsorbed(slab, site_idx)
			
			# Save atoms object to database
			db_ads.write(atoms, slab_idx=slab_idx, site_idx=site_idx)

# Connect to input and output databases
with connect('single_element_COOH_COH_adsorbed.db') as db_ads,\
	...
	for metal in metals:
		...
		# Get atoms with COOH added
		atoms = add_COOH_CO_adsorbed(slab)
